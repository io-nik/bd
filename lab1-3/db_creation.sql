...
CREATE TABLE contract ( -- E6
	...
	CONSTRAINT chk_contract_install_logic CHECK (
		(contract_type = "one-time-pay" AND installment_month IS NULL)
		OR (contract_type = "installment" AND installment_month IS NOT NULL
		AND installment_month BETWEEN 2 AND 12) )
);
...
CREATE TABLE contract_payment ( -- E17
	PRIMARY KEY (pay_doc_id, contract_id),
	pay_doc_id bigint NOT NULL,
...
CREATE TABLE worker_role ( -- E18
	PRIMARY KEY (worker_id, job_pos_id),
