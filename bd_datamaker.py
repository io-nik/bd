...
for contract_id in range(1, NUM_CONTRACTS + 1):
    ...
    statements.append(
        "INSERT INTO contract (client_id, admin_employee_id, summ, abon_id, date_sign) VALUES "
...
for _ in range(NUM_TRAINING_VISITS):
    ...
    if reason == "purchase":
        same_client_docs = [d for d in payment_documents if d["client_id"] == client["client_id"]]
        if same_client_docs:
